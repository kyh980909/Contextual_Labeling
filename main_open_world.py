...
def get_args_parser():
    ...
    parser.add_argument('--lr_backbone', default=4e-6, type=float)
...
def main(args):
    if len(args.wandb_project)>0:
        if len(args.wandb_name)>0:
            wandb.init(project=args.wandb_project, entity="yhkim", group=args.wandb_name)
        else:
            wandb.init(project=args.wandb_project, entity="yhkim")
        wandb.config = args
    #else:
    #    wandb=None
